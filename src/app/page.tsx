    ...
    <div className="flex flex-col bg-[#0B132B] min-h-screen text-white overflow-x-hidden font-sans">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-orange-500/10 via-[#0B132B] to-[#0B132B] px-4 py-20 border-b border-white/5">
        ...
        <div className="max-w-5xl mx-auto text-center z-10 space-y-8 animate-fade-in">
          ...
          <h1 className="text-4xl md:text-7xl font-black tracking-tight leading-tight">
            ...
            <span className="bg-gradient-to-r from-[#F15A24] to-[#FF8C00] bg-clip-text text-transparent animate-pulse">
              Fast Rescue & Care
            </span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative rounded-3xl overflow-hidden border border-white/10 shadow-2xl group">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-slate-900/80 backdrop-blur border border-white/5 p-6 rounded-2xl">
              <h4 className="font-bold text-white mb-1">{t("home.activeShelter")}</h4>
              <p className="text-xs text-white/70">{t("home.shelterDesc")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gradient-to-b from-[#0B132B] to-[#060B18] py-20 px-4 border-y border-white/5">
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { label: t("home.statsRescued"), value: stats.rescued.toLocaleString(), icon: Ambulance },
            { label: t("home.statsTeams"), value: stats.teams, icon: ShieldCheck },
            { label: t("home.statsDonations"), value: `₹${stats.donations.toLocaleString()}`, icon: HeartHandshake },
            { label: t("home.statsCities"), value: stats.cities, icon: Sparkles },
          ].map((stat, idx) => (
      ...
      <section id="how-it-works" className="py-24 px-4 max-w-7xl mx-auto text-center space-y-16">
        <div className="space-y-4">
          ...
          <h2 className="text-3xl md:text-5xl font-black">{t("nav.howItWorks")}</h2>
          ...
          <div className="bg-slate-900 border border-white/10 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            {donationSuccess && (
              <div className="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-400 text-xs font-bold flex items-center gap-2 animate-fade-in">
                ...
                {t("home.thankYouDonation")}
              </div>
            )}

            <form onSubmit={handleDonationSubmit} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-white/60 uppercase mb-1.5">{t("home.donorName")}</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-white/60 uppercase mb-1.5">{t("home.mobileNumber")}</label>
                <input
                  type="tel"
                  required
                  pattern="[0-9]{10}"
                  placeholder={t("home.placeholderMobile")}
              ...
              <div>
                <label className="block text-xs font-bold text-white/60 uppercase mb-1.5">{t("home.donationAmount")}</label>
          ...
          <div className="bg-slate-900 border border-white/20 max-w-md w-full rounded-3xl p-6 shadow-2xl relative">
            ...
            <div className="space-y-4 text-center">
              ...
              <div className="bg-orange-500/10 border border-orange-500/20 text-orange-400 p-4 rounded-xl text-left text-xs space-y-1">
                <span className="block font-bold">{t("home.paymentDetails")}</span>
                <span className="block text-white/60">{t("home.transactionSuccessful")}</span>
              </div>
              ...
              >
                {isSubmitting ? t("home.submitting") : "Confirm & Pay (Simulate Success)"}
              </button>
            </div>
          </div>
        </div>
